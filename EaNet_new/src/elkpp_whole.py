...
class ELKPPNet(nn.Module):
    def __init__(self, cfg, bashArgs, *args, **kwargs):
        super(ELKPPNet, self).__init__()
        if(bashArgs.Backbone=="Resnet18"):
            self.backbone = Resnet18(stride=16)
            self.lkpp = LKPP(in_chan=512, out_chan=256, mode='parallel', with_gp=cfg.aspp_global_feature)
            self.decoder = Decoder(cfg.n_classes, low_chan=[256, 128, 64])
