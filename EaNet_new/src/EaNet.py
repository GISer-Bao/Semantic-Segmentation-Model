...
class EaNet(nn.Module):
    def __init__(self, in_channel=3, n_classes=2, *args, **kwargs):
        super(EaNet, self).__init__()
        self.backbone = Resnet101(in_channel=in_channel, stride=16)
        self.lkpp = LKPP(in_chan=2048, out_chan=256, mode='parallel', with_gp=True)
        self.decoder = Decoder(n_classes, low_chan=[1024, 512, 256])
...
if __name__ == "__main__":
    ...
    net = EaNet(in_channel=3, n_classes=2)
    net.cuda()
    net.train()
    #net = nn.DataParallel(net)
    for i in range(10):
        #  with torch.no_grad():
        in_ten = torch.randn((1, 3, 492, 492)).cuda()
        logits = net(in_ten)
        print(i)
        print(logits.size())
