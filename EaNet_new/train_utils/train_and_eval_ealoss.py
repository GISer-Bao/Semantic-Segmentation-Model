...
# 方案一
def criterion(inputs, target, n_min: int=58000, num_classes: int = 2, ignore_index: int = -100):
...
def evaluate(model, data_loader, device, n_min, num_classes=2):
    ...
    with torch.no_grad():
        for images, targets in metric_logger.log_every(data_loader, 100, header):
            ...
            loss = criterion(inputs=output, target=targets.squeeze(1).to(torch.long), 
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, lr_scheduler, n_min, num_classes=2, print_freq=10, scaler=None):
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            ...
            loss = criterion(inputs=output, target=target.squeeze(1).to(torch.long), 
                             n_min=n_min, num_classes=num_classes, ignore_index=255)
